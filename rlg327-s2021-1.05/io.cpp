...
void teleport_mode(dungeon_t *d){
  ...
  while(1){
    switch (getch()) {
    case KEY_UP:
      dest[dim_y] = dest[dim_y]-1;
      render_teleport_dungeon(d,dest[dim_x],dest[dim_y]);
      break;
    case KEY_DOWN:
      dest[dim_y] = dest[dim_y]+1;
      render_teleport_dungeon(d,dest[dim_x],dest[dim_y]);
...
void io_handle_input(dungeon_t *d)
{
  ...
  do {
    ...
    default:
       ...
       * octal, with the leading zero, because ncurses.hpp lists codes in *
